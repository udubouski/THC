...
{
    ALOGE("THX, Downmix_validChannelMask(), call");
    ALOGE("THX, Downmix_validChannelMask(), mask = %u",mask);
    if (!mask) {
        return false;
    }
    // check against unsupported channels
    ...
    // verify has FL/FR
    ALOGE("THX, Downmix_validChannelMask(), AUDIO_CHANNEL_OUT_STEREO = %u",AUDIO_CHANNEL_OUT_STEREO);
    if ((mask & AUDIO_CHANNEL_OUT_STEREO) != AUDIO_CHANNEL_OUT_STEREO) {
        ALOGE("Front channels must be present");
        return false;
    }
...
int32_t DownmixLib_Create(const effect_uuid_t *uuid,
    ...
    ALOGE("THX, DownmixLib_Create(), call"); 
#ifdef DOWNMIX_TEST_CHANNEL_INDEX
...
#endif
    //ALOGE("THX, DownmixLib_Create(), pHandle = %u",pHandle);
    if (pHandle == NULL || uuid == NULL) {
        return -EINVAL;
    }

    ALOGE("THX, DownmixLib_Create(),number of effects in this library, kNbEffects = %d",kNbEffects);
    ...
    ALOGE("THX, DownmixLib_Create(),module->context.state = %u",module->context.state);
    ALOGE("THX, DownmixLib_Create(),module->context.apply_volume_correction = %u",module->context.apply_volume_correction);
    ALOGE("THX, DownmixLib_Create(),module->context.input_channel_count = %u",module->context.input_channel_count);
...
int32_t DownmixLib_Release(effect_handle_t handle) {
    ...
    ALOGE("THX, Downmix_Release(), call");
    ...
    ALOGE("THX, DownmixLib_Release(),pDwmModule->context.state = %u",pDwmModule->context.state);
    free(pDwmModule);
    return 0;
}
...
int32_t DownmixLib_GetDescriptor(const effect_uuid_t *uuid, effect_descriptor_t *pDescriptor) {
    ...
    ALOGE("THX, DownmixLib_GetDescriptor(), call");
    int i;
    ALOGE("THX, DownmixLib_GetDescriptor(),*pDescriptor) = %p",pDescriptor);
    ALOGE("THX, DownmixLib_GetDescriptor(),*uuid) = %p",uuid);
    if (pDescriptor == NULL || uuid == NULL){
        ALOGE("DownmixLib_Create() called with NULL pointer");
        return -EINVAL;
    }
    ALOGE("THX, DownmixLib_GetDescriptor(),kNbEffects = %u",kNbEffects);
...
static int Downmix_Command(effect_handle_t self, uint32_t cmdCode, uint32_t cmdSize,
    ...
    ALOGE("THX, Downmix_Command(), call");
    downmix_module_t *pDwmModule = (downmix_module_t *) self;
    downmix_object_t *pDownmixer;
    ...
    ALOGE("THX, Downmix_Command(), cmdCode = %u", cmdCode);
    switch (cmdCode) {
    case EFFECT_CMD_INIT:
        ALOGE("THX, Downmix_Command(), EFFECT_CMD_INIT");
    ...
    case EFFECT_CMD_SET_CONFIG:
        ALOGE("THX, Downmix_Command(), EFFECT_CMD_SET_CONFIG");
    ...
    case EFFECT_CMD_RESET:
        ALOGE("THX, Downmix_Command(), EFFECT_CMD_RESET");
        Downmix_Reset(pDownmixer, false);
        break;

    case EFFECT_CMD_GET_PARAM:
        ALOGE("THX, Downmix_Command(), EFFECT_CMD_GET_PARAM");
    ...
    case EFFECT_CMD_SET_PARAM:
        ALOGE("THX, Downmix_Command(), EFFECT_CMD_SET_PARAM");
    ...
    case EFFECT_CMD_DISABLE:
        ALOGE("THX, Downmix_Command(), EFFECT_CMD_DISABLE");
        if (pReplyData == NULL || replySize == NULL || *replySize != sizeof(int)) {
            return -EINVAL;
        }
    ...
    case EFFECT_CMD_SET_DEVICE:
        ALOGE("THX, Downmix_Command(), EFFECT_CMD_SET_DEVICE");
    ...
    case EFFECT_CMD_SET_VOLUME: {
        ALOGE("THX, Downmix_Command(), EFFECT_CMD_SET_VOLUME");
    ...
    case EFFECT_CMD_SET_AUDIO_MODE:
        ALOGE("THX, Downmix_Command(), EFFECT_CMD_SET_AUDIO_MODE");
        if (pCmdData == NULL || cmdSize != (int)sizeof(uint32_t)) {
            return -EINVAL;
        }
    ...
    default:
        ALOGE("THX, Downmix_Command(), default_case");
...
{
    ALOGE("THX, Downmix_GetDescriptor(), call");
...
int Downmix_Init(downmix_module_t *pDwmModule) {
    ...
    ALOGE("THX, Downmix_Init(), pDwmModule->config.inputCfg.accessMode = %u",pDwmModule->config.inputCfg.accessMode);
    pDwmModule->config.inputCfg.format = AUDIO_FORMAT_PCM_16_BIT;
    ALOGE("THX, Downmix_Init(), pDwmModule->config.inputCfg.format = %u",pDwmModule->config.inputCfg.format);
    pDwmModule->config.inputCfg.channels = AUDIO_CHANNEL_OUT_7POINT1;
    ALOGE("THX, Downmix_Init(), pDwmModule->config.inputCfg.channels = %u",pDwmModule->config.inputCfg.channels);
    ...
    ALOGE("THX, Downmix_Init(), pDwmModule->config.inputCfg.mask = %u",pDwmModule->config.inputCfg.mask);

    pDwmModule->config.inputCfg.samplingRate = 44100;
    ALOGE("THX, Downmix_Init(), pDwmModule->config.inputCfg.samplingRate = %u",pDwmModule->config.inputCfg.samplingRate);
    pDwmModule->config.outputCfg.samplingRate = pDwmModule->config.inputCfg.samplingRate;
    ALOGE("THX, Downmix_Init(), pDwmModule->config.outputCfg.samplingRate = %u",pDwmModule->config.outputCfg.samplingRate);

    // set a default value for the access mode, but should be overwritten by caller
    pDwmModule->config.outputCfg.accessMode = EFFECT_BUFFER_ACCESS_ACCUMULATE;
    ALOGE("THX, Downmix_Init(), pDwmModule->config.outputCfg.accessMode = %u",pDwmModule->config.outputCfg.accessMode);
    pDwmModule->config.outputCfg.format = AUDIO_FORMAT_PCM_16_BIT;
    ALOGE("THX, Downmix_Init(), pDwmModule->config.outputCfg.format = %u",pDwmModule->config.outputCfg.format);
    pDwmModule->config.outputCfg.channels = AUDIO_CHANNEL_OUT_STEREO;
    ALOGE("THX, Downmix_Init(), pDwmModule->config.outputCfg.channels = %u",pDwmModule->config.outputCfg.channels);
    ...
    ALOGE("THX, Downmix_Init(), pDwmModule->config.outputCfg.mask = %u",pDwmModule->config.outputCfg.mask);
    ...
    ALOGE("THX, Downmix_Init(), pDwmModule->context.state = %u",pDwmModule->context.state);
...
int Downmix_Configure(downmix_module_t *pDwmModule, effect_config_t *pConfig, bool init) {
    ALOGE("THX, Downmix_Configure(), call");
    downmix_object_t *pDownmixer = &pDwmModule->context;

    ALOGE("THX, Downmix_Configure(), pConfig->inputCfg.samplingRate = %u",pConfig->inputCfg.samplingRate);
    ALOGE("THX, Downmix_Configure(), pConfig->inputCfg.samplingRate = %u",pConfig->outputCfg.samplingRate);
    ALOGE("THX, Downmix_Configure(), pConfig->outputCfg.channels = %u",pConfig->outputCfg.channels);
    ALOGE("THX, Downmix_Configure(), pConfig->inputCfg.format = %u",pConfig->inputCfg.format);
    ALOGE("THX, Downmix_Configure(), pConfig->outputCfg.format = %u",pConfig->outputCfg.format);
    ...
    ALOGE("THX, Downmix_Configure(), pDownmixer->input_channel_count = %u",pDownmixer->input_channel_count);        
    }
...
int Downmix_setParameter(downmix_object_t *pDownmixer, int32_t param, uint32_t size, void *pValue) {
    ALOGE("THX, Downmix_setParameter(), call");
    int16_t value16;
    ...
    ALOGE("THX, Downmix_setParameter(), switch (param)");
    switch (param) {

      case DOWNMIX_PARAM_TYPE:
        ALOGE("THX, Downmix_setParameter(), DOWNMIX_PARAM_TYPE");
        ...
        ALOGE("THX, Downmix_setParameter(), value16 = %u",value16);   
        ...
        if (!((value16 > DOWNMIX_TYPE_INVALID) && (value16 <= DOWNMIX_TYPE_LAST))) {
            ...
            ALOGE("THX, Downmix_setParameter(), pDownmixer->type= %u",pDownmixer->type);   
        break;

      default:
        ALOGE("THX, Downmix_setParameter(), default_case");
...
int Downmix_getParameter(downmix_object_t *pDownmixer, int32_t param, uint32_t *pSize, void *pValue) {
    ALOGE("THX, Downmix_getParameter(), call");
    int16_t *pValue16;

    ALOGE("THX, Downmix_getParameter(), switch (param)");
    switch (param) {

    case DOWNMIX_PARAM_TYPE:
        ALOGE("THX, Downmix_getParameter(), DOWNMIX_PARAM_TYPE");
      ...
      *pSize = sizeof(int16_t);
      ALOGE("THX, Downmix_setParameter(), *pValue16 = %u",*pValue16);   
      ALOGE("THX, Downmix_setParameter(),*pSize = %u",*pSize);   
      ALOGV("Downmix_getParameter DOWNMIX_PARAM_TYPE is %" PRId16, *pValue16);
      break;

    default:
      ALOGE("THX, Downmix_getParameter(), default_case");
      ALOGE("THX, Downmix_setParameter(), param = %u",param); 
...
bool Downmix_foldGeneric(
    ...
    ALOGE("THX, Downmix_foldGeneric(), mask = %u",mask); 
    const bool hasSides = (mask & kSides) != 0;
    const bool hasBacks = (mask & kBacks) != 0;

    const int numChan = audio_channel_count_from_out_mask(mask);
    ALOGE("THX, Downmix_foldGeneric(), numChan = %u",numChan); 
    const bool hasFC = ((mask & AUDIO_CHANNEL_OUT_FRONT_CENTER) == AUDIO_CHANNEL_OUT_FRONT_CENTER);
    const bool hasLFE =
            ((mask & AUDIO_CHANNEL_OUT_LOW_FREQUENCY) == AUDIO_CHANNEL_OUT_LOW_FREQUENCY);
    const bool hasBC = ((mask & AUDIO_CHANNEL_OUT_BACK_CENTER) == AUDIO_CHANNEL_OUT_BACK_CENTER);
